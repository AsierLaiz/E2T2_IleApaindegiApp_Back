...
class HitzorduaController extends Controller
{
    ...
    public function destroy($id): JsonResponse
    {
        try {
            ...
            if (!\DB::table('hitzorduak')->where('id', $id)->exists()) {
                return response()->json(['message' => 'Cita no encontrada'], 404);
            }
            
            // Eliminar relaciones con servicios
            \DB::table('hitzorduak_zerbitzuak')->where('hitzordua_id', $id)->delete();
            
            // Eliminar la cita
            \DB::table('hitzorduak')->where('id', $id)->delete();
            
            return response()->json(['message' => 'Hitzordua eliminada correctamente'], 200);
    ...
    {
        $hitzorduak = Hitzordua::where('bezeroa_id', $bezeroa_id)
            ->whereNull('deleted_at')
            ->with('bezeroa', 'ikaslea', 'zerbitzuak')
            ->get();
        return response()->json($hitzorduak, 200);
    }
}
